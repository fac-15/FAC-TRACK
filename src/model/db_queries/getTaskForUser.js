...
const getTaskForUser = (username, weekId) => {
  return new Promise((resolve, reject) => {
      db.query(`SELECT completion, confidence, name from logs l inner join users u on u.id = l.user_id INNER JOIN tasks t on l.task_id = t.id INNER JOIN weeks w on t.week_id = w.id  WHERE username = '${username}' and week_id = ${weekId}`,
      (err, res) => {
          if (err) {
